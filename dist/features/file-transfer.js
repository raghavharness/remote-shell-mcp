...
export class FileTransfer {
     ...
     */
    async download(session, options) {
        const startTime = Date.now();
        const localPath = options.localPath.replace(/^~/, homedir());
        try {
            if (session.type === "ssh2" && session.sshClient) {
     ...
     */
    async uploadViaSftp(client, localPath, remotePath, startTime) {
     ...
     */
    async downloadViaSftp(client, remotePath, localPath, startTime) {
        return new Promise((resolve) => {
            client.sftp((err, sftp) => {
                if (err) {
                    resolve({
     ...
     */
    async uploadViaScp(session, localPath, remotePath, startTime) {
     ...
     */
    async downloadViaScp(session, remotePath, localPath, startTime) {
        return new Promise(async (resolve) => {
            if (!session.childProcess || !session.childProcess.stdin) {
                resolve({
                    success: false,
     ...
     */
    async listRemote(session, remotePath) {
    ...
    async listViaSftp(client, remotePath) {
        return new Promise((resolve) => {
            client.sftp((err, sftp) => {
                if (err) {
                    resolve([]);
