...
export class BlockManager {
    blocks = new Map();
    sessionBlocks = new Map(); // sessionId -> blockIds
    blockCounter = 0;
    maxBlocksPerSession = 100;
    /**
     * Create a new block for a command execution
     */
    createBlock(sessionId, command, workingDirectory) {
     ...
     */
    completeBlock(blockId, output, exitCode, isError = false) {
     ...
     */
    getSessionBlocks(sessionId, limit) {
    ...
    searchBlocks(options) {
        ...
        if (tags && tags.length > 0) {
            blocksToSearch = blocksToSearch.filter(block => tags.some(tag => block.tags.includes(tag)));
        }
        const results = [];
     ...
     */
    getBlockOutput(blockId, stripAnsiCodes = true) {
        const block = this.blocks.get(blockId);
        if (!block)
            return null;
