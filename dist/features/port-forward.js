...
export class PortForwarder {
     ...
     */
    async startLocalForward(session, options) {
     ...
     */
    async startRemoteForward(session, options) {
        const { localPort, remoteHost = "localhost", remotePort } = options;
        if (!remotePort) {
     ...
     */
    async startDynamicForward(session, localPort) {
        const forwardId = `fwd-${++this.forwardCounter}`;
        const portForward = {
            id: forwardId,
            type: "dynamic",
        ...
        throw new Error("Dynamic forwarding not yet implemented. Use: ssh -D " + localPort + " for SOCKS proxy.");
    }
    /**
     * Stop a port forward
     */
    async stopForward(session, forwardId) {
     ...
     */
    async startSsh2LocalForward(client, portForward) {
        return new Promise((resolve, reject) => {
            const server = createServer((socket) => {
                ...
                client.forwardOut("127.0.0.1", portForward.localPort, portForward.remoteHost, portForward.remotePort, (err, stream) => {
                    if (err) {
                        socket.end();
                        return;
                    }
     ...
     */
    async startSsh2RemoteForward(client, portForward) {
        return new Promise((resolve, reject) => {
     ...
     */
    async startShellLocalForward(session, portForward) {
        ...
        throw new Error("Could not extract connection details for port forwarding. " +
