...
export class ShareServer extends EventEmitter {
     ...
     */
    async handleInterrupt(ws, client) {
     ...
     */
    async handleCommand(ws, client, command) {
        if (!client.authenticated) {
     ...
     */
    async handleTabComplete(ws, client, partial) {
        if (!client.authenticated || client.permissions !== "control") {
            return;
        }
        const share = shareManager.getShare(client.shareId);
     ...
     */
    async handleRawInput(ws, client, data) {
        if (!client.authenticated || client.permissions !== "control") {
            return;
        }
        const share = shareManager.getShare(client.shareId);
     ...
     */
    sendOutputHistory(ws, sessionId) {
        const session = sessionManager.getSession(sessionId);
        if (!session)
            return;
     ...
     */
    broadcastDirectoryChange(shareId, directory) {
     ...
     */
    broadcastUserChange(shareId, user) {
        for (const [ws, client] of this.clients) {
            if (client.shareId === shareId && client.authenticated && ws.readyState === WebSocket.OPEN) {
                this.sendToClient(ws, {
                    type: "user-change",
     ...
     */
    broadcastCommandComplete(shareId) {
        for (const [ws, client] of this.clients) {
            if (client.shareId === shareId && client.authenticated && ws.readyState === WebSocket.OPEN) {
                this.sendToClient(ws, {
     ...
     */
    broadcastContextUpdate(shareId, context) {
        for (const [ws, client] of this.clients) {
            if (client.shareId === shareId && client.authenticated && ws.readyState === WebSocket.OPEN) {
                this.sendToClient(ws, {
                    type: "context-update",
     ...
     */
    broadcastSessionEnded(shareId, reason = "Session ended") {
        for (const [ws, client] of this.clients) {
     ...
     */
    getShareHtml(shareId, permissions) {
        return `<!DOCTYPE html>
<html>
<head>
