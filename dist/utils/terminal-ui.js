 ...
 */
function horizontalLine(width, title) {
    if (!title) {
    ...
    return (BOX.horizontal.repeat(leftPart) +
        titleWithSpaces +
 ...
 */
function formatStatusBar(workingDir, connected, uptime, width) {
...
export function formatBoxHeader(options) {
    ...
    const title = options.sessionName
        ...
        : undefined;
    lines.push(`${COLORS.gray}${BOX.topLeft}${horizontalLine(innerWidth, title)}${BOX.topRight}${COLORS.reset}`);
    // Status bar (if we have session info)
    if (options.workingDir !== undefined) {
        const statusContent = formatStatusBar(options.workingDir, options.connected ?? false, options.uptime, innerWidth);
        lines.push(`${COLORS.gray}${BOX.vertical}${COLORS.reset} ${padToWidth(statusContent, innerWidth - 1)}${COLORS.gray}${BOX.vertical}${COLORS.reset}`);
        // Separator
        lines.push(`${COLORS.gray}${BOX.leftT}${BOX.horizontal.repeat(innerWidth)}${BOX.rightT}${COLORS.reset}`);
    }
    return lines.join("\n");
}
/**
 ...
 */
export function formatBoxCommand(command, width) {
    const w = width || 60;
 ...
 * Create a box footer
 */
export function formatBoxFooter(width) {
    const w = width || 60;
    const innerWidth = w - 2;
 ...
 */
export function formatTerminalPrompt(sessionName, workingDir, command) {
 ...
 */
export function formatSessionHeader(sessionName, workingDir, connected, command, uptime) {
    const width = 60;
    const lines = [];
    lines.push(formatBoxHeader({
        sessionName,
        workingDir,
        connected,
        uptime,
 ...
 */
export function formatOutputContent(output) {
    return output;
}
/**
 * Format an error block with context for AI analysis
 */
export function formatErrorBlock(command, exitCode, stderr, context) {
    const lines = [];
    const width = 60;
    const innerWidth = width - 2;
    // Error header
    lines.push(`${COLORS.gray}${BOX.topLeft}${horizontalLine(innerWidth, `${COLORS.red}${COLORS.bold}${ICONS.error} Error${COLORS.reset}`)}${BOX.topRight}${COLORS.reset}`);
    // Exit code
    if (exitCode !== undefined) {
        const exitLine = `Exit code: ${COLORS.red}${exitCode}${COLORS.reset}`;
        lines.push(`${COLORS.gray}${BOX.vertical}${COLORS.reset} ${padToWidth(exitLine, innerWidth - 1)}${COLORS.gray}${BOX.vertical}${COLORS.reset}`);
    }
    // Error type if detected
    if (context.errorType) {
        const typeLine = `Type: ${COLORS.yellow}${context.errorType}${COLORS.reset}`;
        lines.push(`${COLORS.gray}${BOX.vertical}${COLORS.reset} ${padToWidth(typeLine, innerWidth - 1)}${COLORS.gray}${BOX.vertical}${COLORS.reset}`);
    }
    ...
    for (const line of stderrLines) {
        lines.push(`${COLORS.gray}${BOX.vertical}${COLORS.reset} ${COLORS.red}${line}${COLORS.reset}`);
    }
    // Footer
    lines.push(`${COLORS.gray}${BOX.bottomLeft}${BOX.horizontal.repeat(innerWidth)}${BOX.bottomRight}${COLORS.reset}`);
    // Suggestion section (for AI to read)
    if (context.suggestedFix) {
        lines.push("");
        lines.push(`${COLORS.yellow}${ICONS.suggestion} Suggested fix:${COLORS.reset}`);
        lines.push(`   ${COLORS.cyan}${context.suggestedFix}${COLORS.reset}`);
        if (context.autoFixable) {
            lines.push(`   ${COLORS.green}${ICONS.autoFix} [Auto-fixable]${COLORS.reset}`);
        }
    }
    // Context for AI
