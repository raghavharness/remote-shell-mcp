...
export function isSessionExitCommand(command) {
...
export function isSessionInterruptCommand(command) {
    const trimmed = command.trim();
...
export function isCdCommand(command) {
    const trimmed = command.trim();
    return CD_PATTERNS.some(pattern => pattern.test(trimmed));
}
export function extractCdTarget(command) {
    const trimmed = command.trim();
