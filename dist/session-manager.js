...
export class SessionManager {
     ...
     */
    async startChildProcessSession(command, args, name, description, originalCommand, options = {}) {
     ...
     */
    async execSsh2Command(session, command, timeout) {
     ...
     */
    async execChildProcessCommand(session, command, waitTime) {
     ...
     */
    async sendSignal(session, signal) {
        if (session.type === "child_process" && session.childProcess) {
            if (signal === "SIGINT") {
                session.childProcess.stdin?.write("\x03");
            }
     ...
     */
    async updateWorkingDirectory(session) {
        try {
            if (session.type === "child_process" && session.childProcess?.stdin) {
                session.outputBuffer = [];
     ...
     */
    async updateCurrentUser(session) {
        try {
            if (session.type === "child_process" && session.childProcess?.stdin) {
                session.outputBuffer = [];
     ...
     */
    async waitForConnection(session, timeoutMs = 15000) {
