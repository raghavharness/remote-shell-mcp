...
function formatPrompt(sessionName, command) {
...
function formatOutputStart() {
    return `${COLORS.dim}${COLORS.gray}────────────────────────────────────────${COLORS.reset}`;
}
function formatOutputEnd() {
    return `${COLORS.dim}${COLORS.gray}────────────────────────────────────────${COLORS.reset}`;
}
...
function isSessionInterruptCommand(command) {
    const trimmed = command.trim();
...
function isRemoteShellCommand(command) {
    const trimmed = command.trim();
    return REMOTE_COMMAND_PATTERNS.some((pattern) => pattern.test(trimmed));
}
// Extract a friendly name from the command
function extractSessionName(command) {
    const trimmed = command.trim();
    // gcloud compute ssh instance-name
    const gcloudMatch = trimmed.match(/gcloud\s+compute\s+ssh\s+["']?([^\s"']+)["']?/);
    if (gcloudMatch)
        return `gcloud:${gcloudMatch[1]}`;
...
class RemoteShellServer {
    ...
    async startChildProcessSession(command, args, name, description, originalCommand) {
        ...
        childProc = (0, child_process_1.spawn)(modifiedCommand, [], {
            cwd: (0, os_1.homedir)(),
    ...
    async startSsh2Session(host, username, options) {
        const sessionId = this.generateSessionId();
        ...
        if (options.password) {
            ...
            const keyPath = options.privateKeyPath.replace("~", (0, os_1.homedir)());
            if ((0, fs_1.existsSync)(keyPath)) {
                connectConfig.privateKey = (0, fs_1.readFileSync)(keyPath);
            ...
            const defaultKeys = [
                ...
                (0, path_1.join)((0, os_1.homedir)(), ".ssh", "id_ecdsa"),
            ];
    ...
    async execSsh2Command(session, command, timeout = 30000) {
        return new Promise((resolve, reject) => {
            if (!session.sshClient) {
    ...
    async execChildProcessCommand(session, command, waitTime = 2000) {
...
This creates an authentic terminal experience where the user interprets the output themselves.`,
                    inputSchema: {
                        ...
                        properties: {
                            command: {
                                type: "string",
                                description: "The shell command to execute. Can be an SSH command (starts session), a regular command (runs in active session), or '~.' / '//end' (ends session)",
                            },
                            waitTime: {
                                type: "number",
...
- custom: Any command that opens a shell`,
                ...
                },
                {
                    ...
                    description: "Get status of all remote sessions. Shows active session, connection status, and command counts.",
                    inputSchema: {
                        type: "object",
                        properties: {},
                    },
                ...
                },
                {
                    ...
                    description: "End remote session(s) and return to local context. Use sessionId='all' to end all sessions.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            sessionId: {
...
The remote session has been disconnected. Start a new session to continue.`,
                            ...
                            if (session.type === "ssh2") {
                                output = await this.execSsh2Command(session, trimmedCommand, waitTime || 30000);
                            }
                            else {
                                output = await this.execChildProcessCommand(session, trimmedCommand, waitTime || 2000);
                            }
                            return {
                                content: [
                                    {
...
${formatOutputEnd()}`,
                        ...
                        if (isRemoteShellCommand(trimmedCommand)) {
                            ...
                            const newSession = await this.startChildProcessSession(executable, cmdArgs, sessionName, `Remote session: ${trimmedCommand}`, trimmedCommand);
                    ...
                    case "remote_session_start": {
                        ...
                        switch (method) {
                            ...
                            case "gcloud": {
                                ...
                                session = await this.startChildProcessSession("gcloud", gcloudArgs, sessionName || `gcloud:${instance}`, `GCloud SSH to ${instance}`, cmd);
                                break;
                            }
                            case "aws": {
                                ...
                                session = await this.startChildProcessSession("aws", awsArgs, sessionName || `aws:${targetId}`, `AWS SSM to ${targetId}`, cmd);
                                break;
                            }
                            case "azure": {
                                ...
                                session = await this.startChildProcessSession("az", azArgs, sessionName || `azure:${vmName}`, `Azure SSH to ${vmName}`, cmd);
                                break;
                            }
                            case "custom": {
                                ...
                                session = await this.startChildProcessSession(parts[0], parts.slice(1), sessionName || extractSessionName(command), `Custom: ${command}`, command);
                                break;
                            }
                            default:
                                throw new Error(`Unknown method: ${method}`);
                        }
                        const initialOutput = stripAnsi(session.outputBuffer.join("")).slice(-1500);
                        return {
                            content: [
                                {
                                    type: "text",
