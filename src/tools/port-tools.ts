...
import { PortForward } from "../types.js";
 ...
 */
export async function handleListPortForwards(params: { sessionId?: string }): Promise<ToolResult> {
  ...
  if (!session) {
    ...
    const allForwards: Array<{ session: string; forward: PortForward }> = [];
 ...
 */
export async function handleStopAllPortForwards(params: { sessionId?: string }): Promise<ToolResult> {
  const { sessionId } = params;
 ...
 */
function formatForwardLine(forward: PortForward, sessionName?: string): string {
