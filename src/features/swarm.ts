...
export class SwarmManager {
   ...
   */
  private async startSessionForTarget(
    method: SwarmConnectionMethod,
    target: SwarmTarget,
    options: { autoReconnect?: boolean }
  ) {
    switch (method) {
      case "ssh": {
        ...
        return await sessionManager.startChildProcessSession(
          "ssh",
          [target.username + "@" + target.host],
          `${target.username}@${target.host}`,
          `Swarm SSH to ${target.host}`,
          sshCmd,
          options
        );
      }
      ...
      case "gcloud": {
        ...
        if (target.zone) gcloudArgs.push(`--zone=${target.zone}`);
        if (target.project) gcloudArgs.push(`--project=${target.project}`);

        const cmd = `gcloud ${gcloudArgs.join(" ")}`;
        return await sessionManager.startChildProcessSession(
          "gcloud",
          gcloudArgs,
          `gcloud:${target.instance}`,
          `Swarm GCloud SSH to ${target.instance}`,
          cmd,
          options
        );
      }
      ...
      case "aws": {
        ...
        return await sessionManager.startChildProcessSession(
          ...
          awsArgs,
          `aws:${target.targetId}`,
          `Swarm AWS SSM to ${target.targetId}`,
          cmd,
          options
        );
      }
      ...
      case "azure": {
        ...
        return await sessionManager.startChildProcessSession(
          ...
          azArgs,
          `azure:${target.vmName}`,
          `Swarm Azure SSH to ${target.vmName}`,
          cmd,
          options
        );
      }
  ...
  async execInSwarm(
    ...
    const execOne = async (sessionId: string): Promise<SwarmExecResult> => {
   ...
   */
  async sendInputToSwarm(swarmId: string, input: string): Promise<void> {
    const swarm = this.swarms.get(swarmId);
    if (!swarm) {
      throw new Error(`Swarm not found: ${swarmId}`);
    }
   ...
   */
  async interruptSwarm(swarmId: string): Promise<void> {
    const swarm = this.swarms.get(swarmId);
    if (!swarm) {
      throw new Error(`Swarm not found: ${swarmId}`);
    }
   ...
   */
  async endSwarm(swarmId: string): Promise<number> {
    const swarm = this.swarms.get(swarmId);
    if (!swarm) {
      throw new Error(`Swarm not found: ${swarmId}`);
    }
   ...
   * End all swarms
   */
  async endAllSwarms(): Promise<number> {
    let totalCount = 0;
   ...
   */
  async removeTargetFromSwarm(swarmId: string, targetId: string): Promise<boolean> {
    const swarm = this.swarms.get(swarmId);
    if (!swarm) {
      throw new Error(`Swarm not found: ${swarmId}`);
    }
