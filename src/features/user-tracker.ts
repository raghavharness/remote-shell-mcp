...
export class UserTracker {
  private userStack: Map<string, string[]> = new Map();

  /**
   * Initialize user tracking for a session
   */
  initSession(sessionId: string, initialUser: string = "user"): void {
  ...
  getCurrentUser(sessionId: string): string {
    ...
    if (!stack || stack.length === 0) return "user";
   ...
   */
  private isSuCommand(command: string): boolean {
    const patterns = [
   ...
   */
  private extractSuTarget(command: string): string {
   ...
   */
  private extractWhoamiOutput(output: string): string | null {
   ...
   */
  private extractUserFromPrompt(output: string): string | null {
   ...
   * Broadcast user change to share clients
   */
  private broadcastUserChange(sessionId: string, user: string): void {
